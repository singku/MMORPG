        ...
        google::protobuf::Message* message)
{
    ...
    if (name != "onlineproto.cs_0x0106_player_change_state"
        || name != "onlineproto.sc_0x0107_notify_player_change_state") {
        TRACE_TLOG("PARSE MSG:'%s' ok[msg_len:%u]\nMSG:\n[%s]", 
                name.c_str(), debug_str.size(), debug_str.c_str());
    }
...
#define PARSE_MSG \
    do { \
        ...
        if (player) { \
            cli_in_.SerializeToString(player->temp_info.cache_string); \
        } \
    } while (0)
...
#define PARSE_SVR_MSG(pbuf) \
    do { \
        ...
        if (parse_message(body, bodylen, &pbuf)) {\
            return send_err_to_player(player, \
                    player->cli_wait_cmd, cli_err_svr_proto_format_err);\
        }\
        if (player) { \
            cli_in_.ParseFromString(*(player->temp_info.cache_string)); \
        } \
    } while (0)

#define PARSE_SVR_MSG_WITHOUT_IN(pbuf) \
    do { \
        pbuf.Clear();\
        if (parse_message(body, bodylen, &pbuf)) {\
            return send_err_to_player(player, \
...
#define CACHE_OUT_MSG(msg) \
    do { \
        if (player) { \
            msg.SerializeToString(player->temp_info.cache_out_string); \
        } \
    } while(0)

#define PARSE_OUT_MSG(msg) \
    do { \
        if (player) { \
            msg.ParseFromString(*(player->temp_info.cache_out_string)); \
        } \
    } while(0)
